...
transactions_Router
  .route("/transactions")
  .all(check_user)
  .get(validateQuery(PaginationQuery), get_transactions)
  .post(validateBody(CreateTransactionsRequest), submit_transaction);

transactions_Router
  .route("/transactions/filter")
  .all(check_user)
  .get(validateQuery(TransactionFilterQuery), filter_transactions);
...
transactions_Router
  .route("/transactions/breakdown/category")
  .all(check_user)
  .get(validateQuery(MonthYearQuery), category_breakdown);

transactions_Router
  .route("/transactions/breakdown/monthly")
  .all(check_user)
